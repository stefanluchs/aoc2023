...
data class DayTen(val input: String) : Day<Int> {
        ...
        val enclosed =
            maze
                ...
                .filterNot {
                    ...
                    val tiles = loop.filter { it.asPair() in nodes }.map { it.char }.filterNot { it == '|' }

                    val crossings =
                        tiles.filter { it == '-' }.size +
                            minOf(
    ...
    data class Maze(val tiles: Map<Pair<Int, Int>, Tile>) {
                ...
                val tiles =
                    input
                        ...
                        .mapIndexed { row, chars ->
                            chars
                                .mapIndexed { column, char -> Tile(row, column, char) }
        ...
        fun areas(): List<List<Pair<Int, Int>>> {
            ...
            while (tiles.isNotEmpty()) {
                ...
                while (adjacent.isNotEmpty()) {
                    ...
                    val connectionAdjacent =
                        connection.adjacent().filter { it in tiles }.filterNot { it in visited }.toMutableList()
        ...
        private fun nonLoopTiles(): List<Tile> {
            val loop = loop()

            return this.tiles.entries.asSequence().map { it.value }.filterNot { it in loop }.toList()
        }
        ...
        private fun Pair<Int, Int>.adjacent(): List<Pair<Int, Int>> {
            return listOf(
                first to second + 1,
                first to second - 1,
                first + 1 to second,
